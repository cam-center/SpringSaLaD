...
public class ClusterAnalyzer_OLD {
...
*/
        ...
        ClusterAnalyzer_OLD clusterAnalyzer2 = new ClusterAnalyzer_OLD(filePathStr, true,
    ...
    Trajectory_OLD[] trajectory_OLDs;

    Path outFolder;

    boolean validSimFile = true;

    public ClusterAnalyzer_OLD(String simfile, boolean withMonomer, double[] timePointValues, int[] runNumbers)
            throws IOException{
    ...
    public void init(SimFile simFileObj, boolean withMonomer, double[] timePointValues, int[] runNumbers)
        ...
        trajectory_OLDs = new Trajectory_OLD[runNumbers.length];
        for (int i = 0; i < trajectory_OLDs.length; i++){
            trajectory_OLDs[i] = trajFactory.manufactureTraj(runNumbers[i], inpath);
        }
        //</editor-fold>
    ...
    public void getMeanTrajectory() throws IOException{
        ...
        for (Trajectory_OLD trajectory_OLD: trajectory_OLDs){
            trajStats = trajectory_OLD.gatherClusterStats();
    ...
    public void getSingleTrajectory(int trajNum) throws IOException{
        ...
        //calculations
        TrajStats trajStats = trajectory_OLDs[j].gatherClusterStats();
    ...
    public void getSteadyStateDistribution(double[] SSTimePoints) throws IOException{
        ...
        SortedMap<Cluster_OLD,Integer> compCountMap = createCompCountMap(SSTimePoints);

        SortedMap<Cluster_OLD, Double> compFreqMap = compCountMap.entrySet().stream()
    ...
    private SortedMap<Integer,Integer> createSizeCountMap(double[] SSTimePoints){
        ...
        for (Trajectory_OLD traj: trajectory_OLDs){
    ...
    private SortedMap<Cluster_OLD,Integer> createCompCountMap(double[] SSTimePoints){
        SortedMap<Cluster_OLD,Integer> compCountMap = new TreeMap<>();
        for (Trajectory_OLD traj: trajectory_OLDs){
            List<Cluster_OLD> clusterCompList = traj.gatherClusterCompList(SSTimePoints);
            for (Cluster_OLD cluster_OLD: clusterCompList){
                compCountMap.put(cluster_OLD, compCountMap.containsKey(cluster_OLD) ? compCountMap.get(cluster_OLD)+1 : 1);
            }
        }
        return compCountMap;
    }
