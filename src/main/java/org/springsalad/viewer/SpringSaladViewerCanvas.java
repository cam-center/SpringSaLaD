 ...
 */
/*
 * Ported from VCell (virtualcell/vcell), cbit.vcell.solver.ode.gui.SpringSaladViewerCanvas,
 * also MIT licensed. Changed here: repackaged, and the Colors/NamedColor imports point at this
 * project's own org.springsalad.helpersetup copies rather than VCell's -- VCell's are a verbatim
 * copy of these (same names, same values, same line numbers), so porting them would have created
 * a second Colors class in this project for nothing.
 *
 * Renders with Java2D only. No Java3D, no jogamp.
 */
